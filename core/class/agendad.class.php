...
class agendad extends eqLogic
{
     ...
     */
     ...
     public function collect_agenda()
     {
     	...
     	log::add('agendad', 'info', ' --------INIT collect_agenda-----------');
     	log::add('agendad', 'info', ' ');
        ...
        //initialisation des variables jeedom
     	...
     	$this->checkAndUpdateCmd('alarm', "~~.~~");
     	$this->checkAndUpdateCmd('next_event_start', "");
     	$this->checkAndUpdateCmd('next_event_end', "no event");
     	$this->checkAndUpdateCmd('next_event_subjectn', "");
     	$this->checkAndUpdateCmd('next_event_room', "");
     	$this->checkAndUpdateCmd('next_event_professor', "");
     	$this->checkAndUpdateCmd('current_event_start', "");
     	$this->checkAndUpdateCmd('current_event_end', "no event");
     	$this->checkAndUpdateCmd('current_event_subject', "");
     	$this->checkAndUpdateCmd('current_event_room', "");
     	$this->checkAndUpdateCmd('current_event_professor', "");
     	...
     	$this->checkAndUpdateCmd('current_time_epsi', $time_hours . ":" . $time_minutes);

     	$date_jour = date("m-d-y");
                //force la date pour des tests
                //$date_jour= "09-28-20";
     	$this->checkAndUpdateCmd('current_date_epsi', " " . $date_jour);
     	...
     	if (is_array($jsonData)) {
     		...
     		foreach ($jsonData as $value => $jsonKey) {
     			...
     			foreach ($jsonKey_top as $value2 => $jsonKey2) {
     				...
     				$this->checkAndUpdateCmd('alarm', $alarm_hours . $alarm_minutes);
     			...
     			foreach ($jsonKey as $value2 => $jsonKey2) {
     				...
     				if ($time_hours_minutes_start_resultat > $time_hours_minutes_actuel_resultat) {
     					if ($next_event == "0") {
     						...
     						$next_event = 1;
     						$this->checkAndUpdateCmd('next_event_start', str_replace(":", "", $jsonKey2['start']));
     						$this->checkAndUpdateCmd('next_event_end', str_replace(":", "", $jsonKey2['end']));
     						$this->checkAndUpdateCmd('next_event_subject', $jsonKey2['subject']);
     						$this->checkAndUpdateCmd('next_event_room', $jsonKey2['room']);
     						$this->checkAndUpdateCmd('next_event_professor', $jsonKey2['professor']);
     					} else {
     					...
     					If ($time_hours_minutes_fin_resultat >= $time_hours_minutes_actuel_resultat) {
                           //    $scenario->setlog('Evenement en cours');
     						$this->checkAndUpdateCmd('current_event_start', str_replace(":", "", $jsonKey2['start']));
     						$this->checkAndUpdateCmd('current_event_end', str_replace(":", "", $jsonKey2['end']));
     						$this->checkAndUpdateCmd('current_event_subject', $jsonKey2['subject']);
     						$this->checkAndUpdateCmd('current_event_room', $jsonKey2['room']);
     						$this->checkAndUpdateCmd('current_event_professor', $jsonKey2['professor']);
     					}
     					log::add('agendad', 'info', '');
     				}
     			}
     	...
     	log::add('agendad', 'info', ' ');
     	log::add('agendad', 'info', ' --------END collect_agenda-----------');
     	log::add('agendad', 'info', ' ');
     }
